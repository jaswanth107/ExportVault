...
| Waking a scaled-to-zero worker | API sends one fire-and-forget GET to `WORKER_WAKE_URL` after enqueuing | On hosts with no free worker tier the worker is a web service that spins down, and nothing ever visits its URL to bring it back — so it sleeps permanently and the queue is never consumed. Waking on demand costs one request per export; a keep-alive pinger would cost ~1,440 instance-hours against a 750-hour free allowance. |
...
| 12 | **Every export on the deployed free-tier instance sat at `QUEUED / 0 rows` forever.** Nothing was broken: `/health/ready` reported Postgres, Redis, storage and queue all green, with `waiting: 3, active: 0`. The worker had spun down and nothing existed to wake it — the browser keeps the *API* warm, but no traffic ever reaches the *worker's* URL, so it slept permanently after its first idle timeout. Confirmed by one GET to the worker's health endpoint: it answered with `uptimeSeconds: 9` (the request was its boot) and the queue drained 3 → 0 within 30s. | The API now sends one fire-and-forget GET to `WORKER_WAKE_URL` after enqueuing (`queues/workerWake.ts`). Not awaited — a cold start is ~25s and the create response must not block on it. Failure is logged and swallowed, since the job is already durable in Postgres and Redis by then. Concurrent enqueues join the in-flight request, so a burst causes one boot, not several. Unset ⇒ no-op, which is correct locally and for a real background worker. A keep-alive pinger was rejected: two always-on free services cost ~1,440 instance-hours against a 750-hour allowance. |
| 13 | Related hole found while fixing #12: `enqueueExportJob` returns early when the BullMQ entry is `active`, so a worker that died mid-export left a stale lock that made **Resume a silent no-op** — nothing reclaims that lock while no worker is running, which on a spin-down host is the routine case. | The wake is now sent on that path too, before the early return. A live worker makes it a cheap no-op ping; a dead one gets replaced by a booting worker whose stalled-job checker reclaims the job. Correct under both readings of `active`. |
| 14 | The sign-in password box arrived pre-filled on every visit. Nothing in the app was at fault — `localStorage` holds only the JWT and the React state starts `''` — it was Chrome's password manager filling the field on load. | The field is held read-only across the load window: Chrome skips read-only inputs and deliberately ignores `autocomplete="off"` on password fields, so that attribute is not a fix. The guard lifts on first pointer/focus or after the fill window closes, whichever is first — permanent read-only would also block third-party password managers, which write the value without focusing the input, and `page.fill()`. `autocomplete` stays `current-password` rather than switching to `new-password`, which would suppress the fill but make the browser offer password *generation* on a sign-in form. Six e2e tests cover it, including that signing in still works. |
